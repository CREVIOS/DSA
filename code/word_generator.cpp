...
#include<bits/stdc++.h>
using namespace std;
void generateWords(const vector<char>& chars, int max_length, vector<char>& stack, vector<string>& words) {
    if(stack.size() == max_length) {
        words.push_back(string(stack.begin(), stack.end()));
        return;
    }

    for(char ch : chars) {
        stack.push_back(ch);
        generateWords(chars, max_length, stack, words);
        stack.pop_back();
    }
}
